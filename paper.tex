...
by the \AP{injection rate of} CR energy in the last 650~Myrs. Our simulations show
that \AP{the energy} approximately amounts to 0.03 per cent of the thermal energy inside
...
history of the cluster. \AP{However, since the average injection rates of CR energy are similar, 
the CRes injected during the merger are especially important for the radio emission above 1 GHz 
since these CRs have not yet had time to cool.}  Similarly, the initial CRe distribution is given
...
\C{It is
  ...
  $\alpha_{\rm CR, spat}$ about fiducial values.\footnote{\C{When \C{$\tau_{\rm cl}/\tau_{\rm decay} \sim$const} is adopted, the radial behaviour of $\tau_{\rm cl}$ is
      ...
      effect in Figure~\ref{fig:param_comp_tcl_tD}, we change $\alpha_{\rm CR,
        spat}=1.0\rightarrow0.5$ and \AP{decrease the injection rate of CR energy by a factor five.}}} Instead
...
\caption{\C{Sensitivity of radio emission in the Coma cluster to critical
    ...
    of CRs ($\alpha_{\rm CR, spat}=0.5$) and lower the \AP{injection rate of CR energy by a factor five}; 
    otherwise, we adopt $X_\rmn{tu}=0.2$, $\alpha_\rmn{tu}=0.8$,
    ...
    has a \AP{remarkable} small impact. In the bottom two panels we vary the exponential
