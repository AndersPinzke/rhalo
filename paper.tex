...
\caption{\C{Sensitivity of radio emission in the Coma cluster to critical
    ...
    profiles are mainly determined by the CR distribution, while turbulence has
    a smaller impact. In the bottom two panels we vary the exponential momentum
    growth factor $\tau_{\rm cl}/\tau_{\rm D}$ (see equations~\ref{eq:pexp} and
    \ref{eq:Creacc}), but unlike $X_\rmn{tu}$ should not vary strongly with
    time. Instead, it mainly changes the normalisation of the CR distribution
    and neither spectral shape nor radial synchrotron profile. Interestingly,
    the flat CR profiles adopted here are similar to those required to reproduce
    the radio profiles in our CR streaming model (see
    Figure~\ref{fig:sync_profile}).}}
