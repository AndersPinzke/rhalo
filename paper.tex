...
predictions. We refer the reader to section~\ref{sect:param_comp} for
a minor exploration of parameter space, and leave the full exploration
of parameter space and the interaction between free parameters to
future work.
...
The transport of relativistic electrons and protons in the ICM is a
complex process that depends both on the details of the thermal
component (gas density, temperature, and pressure) as well as
...
\Mprimary (see Figure~\ref{fig:turb} for visualization). Note that in
...
Eqns.~\ref{eq:dpp}-\ref{eq:Gamma_e} we find that $D_\rmn{pp}\propto
I_0/\rho c_s \propto
X_\rmn{tu}^2k_0\epsilon_\rmn{th}^{\alpha_\rmn{tu}-1}\sqrt{T}$. Here we
immediately see that if the turbulent profile trace the thermal energy
(i.e. $\alpha_\rmn{tu}=1$), the square root of temperature alone
determines the diffusion constant. We also see that $D_\rmn{pp}$ is
much more sensitive to $X_\rmn{tu}$ than $k_0$. 
...
exponentially dependent on $D_\rmn{pp}$ as long as $\tau_D \lesssim
\tau_\rmn{cl}$, where $\tau_\rmn{cl}$ represent the duration of
reacceleration of the merger which we assume to be 650~Myrs long.
Note that a fixed reacceleration timescale $\tau_D\sim
1/D_\rmn{pp}\sim 0.2-0.7\,\rmn{Gyr}$ is required to explain the
observations at each radius and for each model (see
Table~\ref{tab:timescales}). This allows us to interchange
$\tau_\rmn{cl} \sim X_\rmn{tu}^2 \sim k_0$, i.e. a longer
...
V/\int_0^{R_{200}}\epsilon_\rmn{th} \rmn{d}V = 0.0003$. The spectrum of
  the initial CRp distribution is determined by the steady state between
...
for $\tau_\rmn{D} = 0.5\,\rmn{Myr}$. This threshold is also seen in
