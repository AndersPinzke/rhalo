...
%\SPO{the derivation of the following is not transparent, could you
%  expand?}  \sout{During reacceleration, the CRps increase their
%  normalization approximately by the factor} \AP{In the region where
%  reacceleration dominates over cooling and injection, the CRps
%%  increase their normalization exponentially. This can easily be
 % understood by considering a simple toy model where a fossil CRp
%  population described by $f=C p^{-\alpha}$ is reaccelerated such that
%  $\tau_D < \tau_\rmn{cooling},\tau_\rmn{injection}$, i.e. injection
%  and cooling can be neglected. The Fokker-Planck equation in
%  Eqn.~\ref{eq:FP_p} then simplifies to}
%\begin{equation}
...
%\end{equation}
%The reaccelerated CRs depend exponentially on $\tau_D\sim 1/D_{pp}$ as
%long as $\tau_D \lesssim \tau_\rmn{cl}$, where $\tau_\rmn{cl}$
%represent the duration of reacceleration of the merger which we assume
%to be 650~Myrs long.
...
rays.
%(note that while radio halos have a non-linear dependence on the overall
%normalization of turbulence, the surface brightness scales linearly with the CR
%abundance; thus, this degree of freedom is easily understood).
