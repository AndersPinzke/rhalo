...
\begin{equation}
k_{\rm cut} \approx A k_L \frac{\langle (\delta \varv_{\rmn{c}})^{2}\rangle^{2}}{c_{\rm s}^{4}}
\label{eqn:k_c} 
\end{equation}
...
\begin{equation}
  ...
  = \cp{2 \left(A \mathcal{M}_\rmn{A}\right)^{-1/2}} \beta = 0.5 \left( \frac{\beta}{50} \right) 
\end{equation}
\cp{[This assumes the energy equipartition argument (by Sarkar) for $D_{pp}^\rmn{MHD}$ and for $D_{pp}^\rmn{HD}$; is this justified at the sme time? if so, why? and we should explain this in a footnote.]}
...
\label{eqn:tau_D} 
\end{equation}
\cp{[The previous equation is wrong (you can see this by checking the units). Instead I get:]
\begin{equation}
\tau_{\rm D} = \frac{p^{2}}{4 D_{pp}} = C_{\rm D} \left(A\mathcal{M}_\rmn{A}\right)^{-1/2} \frac{c}{k_{\rm A}} \frac{c_{\rm s}^{2}}{\varv_{\rm A}^{4}}
\label{eqn:tau_D} 
\end{equation}
}
...
\label{eqn:tau_decay} 
\end{equation}
\cp{[This agrees with Eqn. (3) of \citet{2004ApJ...614..757Y} but appears to disagree with our equation (\ref{eqn:tau_l}): Peng, do you understand this apparent contradiction? Can we motivate this important equation?]}
...
\cp{[I get]
  \begin{eqnarray}
    ...
    \approx\frac{0.6\sqrt{A}\mathcal{M}_\rmn{A}^{7/2}}{C_\rmn{D}\beta}\,\frac{\varv_\rmn{A}^2}{\varv_\rmn{c} c}\nonumber\\
    &\approx&0.1 \left( \frac{c_{\rm s}}{1000 \, {\rm km \, s^{-1}}} \right) \left( \frac{\beta}{100} \right)^{-1}\, 
\label{eqn:ratios_TTD} 
\end{eqnarray}
[which is {\bf not} independent of the properties of turbulence!]}
