...
parameters. We do not trace single Lagrangian particles in the
cluster, instead we use a more simple approach where we solve the
Fokker-Plack equation in static spherical shells for injection,
reacceleration, and losses of the CRs. Once the CRs have been
...
\end{eqnarray}
The virial radius of Coma is given by $R_{200} = 2300\,\rmn{kpc}$.  The
bulk of the CRps are injected by relatively low Mach number shocks and
parametrized by $f_\rmn{CRp,inj}(p) = C_\rmn{CRp,inj}\,p^(-2.5)$. The
CRps trace the thermal gas with $C_\rmn{CRp,inj} \propto
\epsilon_\rmn{th}^{\alpha_\rmn{CR,spat}}$ where the normalization is fixed
by assuming that the injected CR energy in the last 650~Myrs make up
0.03\% of the thermal energy inside the virial radius, i.e.
$\int\epsilon_\rmn{CRp,inj} d\rmn{V}/\int\epsilon_\rmn{th} d\rmn{V} =
0.0003$. The spectrum of the initial CRp distibtion is determined by
is given by the steady state between injection and cooling,
\begin{equation}
 f_{CRp,0} \approx \frac{\int_p^\infty f_\rmn{CRp,inj}(p') 
\rmn{d}p'}{\left|{{dp}\over{dt}}\right|_{\rm C}+\frac{p}{\tau_{\rm had}}}\,,
\end{equation}
where the normalization is determined from $\int\epsilon_\rmn{CRp,inj}
d\rmn{V}/\int\epsilon_\rmn{th} d\rmn{V} = 0.003$. Similarly, the
initial CRe distribution is given by the steady state between cooling
(Coulomb, invers Compton, and synchroton) and injection from
$f_{CRp,0}$. We also include the contineous injection of secondary
CRes during $\tau_\rmn{cl}$. 

The compressible turbulence responsible for reacceleating both the CRps and
CRes is parametrized by $I_0 \propto X_\rmn{tu} (n_e
T)^\alpha_\rmn{tu}$. The diffusion constant $D_\rmn{pp}$ is 
calculated for each radial bin using
Eqns.~\ref{eq:Gamma_e}-\ref{eq:dpp}.

In Fig.~\ref{fig:param_comp} we show how assumptions about turbulence
and CRs impact the radio emission. We find that the level of
turbulence has a large impact on both radio spectra and profile, while
the parameter $\alpha_\rmn{tu}$ that determines the spatial profile of
turbulence mainly influence the radio profiles. Interestingly,
assumptions about CRs have less impact on radio emission. This can be
understood by again looking at the scaling of parameters to the
reacceleration timescale, where $\tau_\rmn{D} \propto
X_\rmn{tu}^2$. The CRps approximately gain a factor $\Delta
C_\rmn{reacc.} \sim
\exp{\frac{(2+\alpha)(1-\alpha)}{4}\frac{\tau_\rmn{cl}}{\tau_\rmn{D}}}$
...
$\tau_\rmn{coul}\approx0.01\,\rmn{Myr}$ is much shorter than
$\tau_\rmn{D}\approx0.5\,\rmn{Myr}$, hence $\Delta C_\rmn{reacc.}$
...
\exp{1.7\frac{\tau_\rmn{cl}}{\tau_\rmn{D}}} \sim 10$ for
...
figures. The difference is due to the shorter reacceleration timescale
that increase the contribution from reaccelerated CRes with a factor
few. \AP{Too complicated, perhaps cut or simplify?}
...
\caption{Sensitivity of radio emission in Coma cluster to critical
  ...
  $\alpha_\rmn{tu}=0.8$, and $\alpha_\rmn{CR,spat}$. Note that the
